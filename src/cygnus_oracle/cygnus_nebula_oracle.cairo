...
// @param new_nebula_oracle The struct containing the initialized oracle info
...
// @param price_denominator Empiric`s key for the denomination this oracle returns the prices in // 28254602066752356
@constructor
...
@view
func version{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() -> (version: felt) {
    return Version.read();
}
...
func get_asset_prices{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    return (token0_price=token0_price, token1_price=token1_price);
}
...
func initialize_oracle{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    // / @custom:event InitializeCygnusOracle
...
func set_oracle_pending_admin{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    // / @custom:error admin_already_set Avoid if new admin is the same address as current admin
    with_attr error_message("empiric_lp_oracle__admin_already_set(new_pending_admin)") {
